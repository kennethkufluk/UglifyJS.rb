...
class Squeezer
  ...
  def getAST
    ...
    return @w.with_walkers({
      ...
      "binary"=> lambda do |ast, op, left, right|
        return when_constant.call([ "binary", op, @walk.call(left), @walk.call(right) ], lambda do |ast, c, v|
          #yes
          return best_of(@walk.call(c), ast)
        end, lambda do |ast, expr|
          #no
          return ast
        end)
      end,
        ...
        while i < statements.length
          ...
          if stat[0] == "if" && !stat[3]
            if stat[2][0] == "return" && stat[2][1].nil?
              a.push(make_if.call(nil, negate(stat[1]), [ "block", statements[i..-1] ]))
              break
            end
            last = last_stat(stat[2])
            if (last[0] == "return" && last[1].nil?)
              a.push(make_if.call(nil, stat[1], [ "block", stat[2][1][0..-1] ], [ "block", statements[i..-1] ]))
              break
            end
          end
          a.push(stat)
  ...
  def make_real_if(c, t, e)
    ...
    if empty(t)
      ...
      (lambda do
        a = ASTCodeGenerator.new(c).generate
        n = negate(c)
        b = ASTCodeGenerator.new(n).generate
  ...
  def best_of(ast1, ast2)
    code1 = ASTCodeGenerator.new(ast1).generate
    code2 = ASTCodeGenerator.new(ast2[0] == "stat" ? ast2[1] : ast2).generate
    # puts "BESTOF #{code1} #{code2}" #leave this comment in
