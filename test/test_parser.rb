...
class ParserTest < Test::Unit::TestCase
  ...
  def test_parser
    ParserTestSuite(lambda do |i, input, desc|
      begin
        parse_rb = Parsejs::Parse.new(input).parse
        puts debug("ok " + i.to_s + ": " + desc)
      rescue Exception => e
        debug("FAIL " + i.to_s + " " + desc + " (" + e.to_s + ")")
      end
    end)
  end
  ...
  def ParserTestSuite(callback)
    ...
    for i in (0...inps.length)
      callback.call(i, inps[i][0], inps[i][1])
    end
  end
